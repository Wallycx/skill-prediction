...
module.exports = function SPCore(mod) {
	...
	mod.hook('C_PLAYER_LOCATION', 5, {order: 10, filter: {fake: null}}, event => {
		if(mod.settings.debug.skills.loc) console.log('Location %d %d (%d %d %d %s) > (%d %d %d)', event.type, event.jumpDistance, Math.round(event.loc.x), Math.round(event.loc.y), Math.round(event.loc.z), degrees(event.w), Math.round(event.dest.x), Math.round(event.dest.y), Math.round(event.dest.z))
